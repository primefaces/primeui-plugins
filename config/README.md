...
`mcp.package` and `mcp.versionRange` are the sole authored MCP package identity and strict bounded compatible major-line range. Every host payload derives its one-argument package spec from this contract.
...
Generated provenance records `mcp.package` and `mcp.versionRange` without claiming an installed version, then closes over the payload by listing every generated JSON document and every ordered hashed skill root. Exact generated-payload validation rejects any missing, stale, or extra file beneath generator-owned roots.

`npm run lock:sources` inspects every declared physical tree under `skills/`, computes each `source.treeHash`, sets `lockState` to `locked`, and removes `unresolvedReason`. Repository URLs, identity/order metadata, skill paths, and plugin versions are preserved; MCP package/range policy remains exclusively in `plugins.json`.
